...
**Read `${CLAUDE_PLUGIN_ROOT}/references/validation-review-protocol.md` before reviewing.** Follow its `initial-audit` or `closure-review` authority exactly; the supplied `review_mode` overrides any broader review language below, including general preferences for false positives over false negatives.
