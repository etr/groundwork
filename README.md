...
Codex exports install the runner at `~/.codex/groundwork-run.js` for user scope or `.codex/groundwork-run.js` for project scope. The runner creates each linked task worktree before planning and runs every phase from its project root. It repeatedly chooses the lowest-numbered currently unblocked task, so dependency constraints take precedence and numeric priority breaks ties. An ordinary phase failure gets a bounded fresh recovery session with the raw diagnostic and observed selected-task state; recovery may repair only that task, never publish, and any repair is runner-sealed then freshly validated. One stronger retry is allowed when state does not progress. Scope escape, Git-control changes, destructive operations, uncertain identity, and genuine product judgment stop safely with the exact worktree and branch preserved.
...
Multiple runner commands may be launched against the same repository. A project lease serializes complete tasks for one project, while different projects may execute model phases concurrently. Startup reads and model phases use a writer-preferred repository reader gate. Linked-worktree registration adds a short registry mutex, so another project can start while a model phase is active without exposing a half-created workspace. Repository configuration changes, publication, worktree removal, and recovery remain writer-exclusive; wait diagnostics identify the holder. If another project advances the base, the stale task integrates it and revalidates before publication.
