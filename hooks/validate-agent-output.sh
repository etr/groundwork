...
# fires on every SubagentStop, so it resolves the directory LAZILY — only
# the rare error path needs it, and the success path must not spawn node.
...
log_error() {
  ...
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] validate-agent-output: $1" >> "${_GW_STATE_DIR}/hook-errors.log" 2>/dev/null || true
}
