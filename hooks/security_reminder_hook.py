...
# Debug log file - use groundwork state directory for consistency
import pathlib
DEBUG_LOG_FILE = str(pathlib.Path.home() / ".claude" / "groundwork-state" / "security-warnings.log")
...
def debug_log(message):
    ...
    try:
        ...
        pathlib.Path(DEBUG_LOG_FILE).parent.mkdir(parents=True, exist_ok=True)
...
Use execFile/execFileSync which don't invoke a shell:
  execFile('command', [userInput], callback)
  execFileSync('command', [userInput])

Key differences:
- exec() spawns a shell and is vulnerable to injection
- execFile() runs the command directly without a shell
- Always pass arguments as an array, never interpolate into strings

Only use exec() if you absolutely need shell features (pipes, redirects) and the input is guaranteed to be safe or properly escaped.""",
    },
    {
...
def check_patterns(file_path, content):
    ...
    # Normalize path properly to handle traversal attempts
...
def main():
    ...
    if not file_path or not file_path.strip():
        debug_log("Empty file path, allowing tool")
        sys.exit(0)  # Allow - can't check without a path
    ...
    if rule_name and reminder:
        # Always block and ask user to confirm
        print(f"{reminder}\n\n**To proceed:** Acknowledge this security consideration and retry.", file=sys.stderr)
        sys.exit(2)  # Block - user must explicitly confirm to proceed
