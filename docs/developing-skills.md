...
Skills produce files. The guard in `tests/path-safety.test.js` enforces the invariants with per-rule scope, stated exactly in the guard header and repeated here: invariants 1 and 3 are corpus-wide text scans over every authored surface (`skills/`, `agents/`, `references/`, `lib/`, `hooks/`, `bin/`, `pi-extension/`, root scripts/docs); invariant 2 is a functional contract check (the resolve CLI must emit absolute normalized bindings — it is not a corpus scan); invariant 4 checks the lock-primitive implementations themselves plus a corpus-wide prohibition on touching shared pointers outside the approved helper. Two contract checks are deliberately targeted at the artifacts they govern: the invocation-unique journal contract (the three journal-authoring skills) and the handoff output suffix (the handoff skill):
...
3. **Invocation-unique journals.** Debug and research journals are per-invocation artifacts: each invocation allocates an atomically unique run directory with `mktemp -d "{{debug_dir}}/${SLUG}-XXXXXX"` (journal at `$RUN_DIR/journal.md`; research equivalent under `{{research_dir}}`), passes the literal generated path to every collaborator, returns it in the final or handoff output, and finds prior runs by slug-prefixed directories when resuming — several matches require the handoff literal path or user selection. Never a fixed slug-only name, never a timestamp+PID-only contract (same-second and PID-reuse collisions), never a shared `current` pointer.
