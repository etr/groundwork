...
const { getProjectRoot } = require('./project-context');
 ...
 */
function detectSpecType(specName, baseDir = getProjectRoot()) {
 ...
 */
function readSpec(specName, baseDir = getProjectRoot()) {
 ...
 */
function findFileContaining(specName, pattern, baseDir = getProjectRoot()) {
  const detection = detectSpecType(specName, baseDir);
 ...
 */
function getWritePath(specName, options = {}, baseDir = getProjectRoot()) {
  const detection = detectSpecType(specName, baseDir);
  const canonicalName = normalizeSpecName(specName);
  const specsDir = path.join(baseDir, DEFAULT_SPECS_DIR);
 ...
 */
function listSpecFiles(specName, baseDir = getProjectRoot()) {
  const detection = detectSpecType(specName, baseDir);
 ...
 */
function detectSpecs(baseDir = getProjectRoot()) {
  return {
