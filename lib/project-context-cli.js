...
function main() {
  ...
  if (command === 'select') {
    ...
    // A direct selector invocation carrying a stable session id pins the
    // per-chat snapshot as part of the same successful selection — the
    // receipt below records that this invocation is the authority for it.
    if (sessionId) context.persistSessionSelection(sessionId, projectName, projectPath);
    const receipt = context.createSelectionReceipt({
      projectName,
    ...
    // The receipt is its own JSON line: the pin hook validates an exact
    // schema, so the receipt must never be merged into the bindings object.
