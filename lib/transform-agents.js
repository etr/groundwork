...
// Reads stdin, writes stdout. Usage: transform-agents.js --target <pi|codex|opencode|kiro|zcode|zcode-plugin>

const targetIdx = process.argv.indexOf('--target');
if (targetIdx === -1 || targetIdx + 1 >= process.argv.length) {
  process.stderr.write('Usage: transform-agents.js --target <pi|codex|opencode|kiro|zcode>\n');
  process.exit(1);
}
...
function parseAgentText(text) {
  ...
  while (i < inner.length) {
    ...
    params[key] = unescapeAgentString(inner.substring(valueStart, i));
...
function unescapeAgentString(value) {
  let result = '';
...
function installedRef(agentName, tgt) {
  switch (tgt) {
    ...
    case 'opencode':
    case 'kiro':
    case 'zcode-plugin': // marketplace plugin ships native agents/<name>.md
      return agentName;
    default: // pi, zcode — agents install as review-* skills
...
function formatSingleAgent(call, tgt) {
  ...
  if (tgt === 'codex' && isGroundwork && agentName) {
    const lines = [`> Spawn the \`${agentName}\` custom agent for **${desc}** with \`fork_turns="none"\` and this task:`, '>'];
    if (prompt && prompt !== '...') {
  ...
  if (tgt === 'codex') {
    const executionPolicy = agentName === 'Plan'
      ? 'with `fork_turns="none"`, model `gpt-5.6-terra` at `high` effort'
      : 'with `fork_turns="none"`';
    const lines = [`> Spawn a default subagent for **${desc}** ${executionPolicy} and this task:`, '>'];
    if (prompt && prompt !== '...') {
      for (const pLine of prompt.split('\n')) {
        lines.push(pLine ? `> ${pLine}` : '>');
      }
  ...
  // ZCode + groundwork agent → delegate to a general-purpose subagent that
  // loads the exported review-* skill (ZCode loads custom agents only from
  // marketplace plugins, so the Agent() call itself is not portable)
  if (tgt === 'zcode' && isGroundwork && agentName) {
    const lines = [
      `> Delegate to a subagent: spawn a general-purpose subagent instructed to follow the \`${installedRef(agentName, tgt)}\` skill for **${desc}** with this task:`,
      '>',
    ];
    if (prompt && prompt !== '...') {
      for (const pLine of prompt.split('\n')) {
  ...
  // ZCode marketplace plugin + groundwork agent → the plugin ships the agent
  // natively (agents/<name>.md), so delegate by its bare agent name
  if (tgt === 'zcode-plugin' && isGroundwork && agentName) {
    const lines = [
      `> Spawn the \`${agentName}\` agent for **${desc}** with this task:`,
      '>',
    ];
    if (prompt && prompt !== '...') {
      for (const pLine of prompt.split('\n')) {
...
function formatMultipleAgents(calls, tgt) {
  ...
  if (tgt === 'codex' && calls.every(c => extractAgentInfo(c.subagentType).isGroundwork)) {
    const lines = ['> Spawn these custom agents in parallel, each with `fork_turns="none"`:'];
    for (const call of calls) {
      const { agentName } = extractAgentInfo(call.subagentType);
      const desc = call.description || agentName || 'Review';
      lines.push(`> - **${desc}** (custom agent: \`${agentName}\`)`);
    }
    lines.push('');
    return lines;
  }
  ...
  let header;
  if (tgt === 'zcode') {
    header = '> Spawn these reviews in parallel as general-purpose subagents (each instructed to follow the named skill), or run them in sequence:';
  } else if (tgt === 'zcode-plugin') {
    header = '> Spawn these agents in parallel (one Agent call each), or run them in sequence:';
  } else {
    header = '> Perform these reviews in sequence:';
  }
  const lines = [header];
  for (const call of calls) {
    const { isGroundwork, agentName } = extractAgentInfo(call.subagentType);
