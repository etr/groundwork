...
function main() {
  ...
  console.log(JSON.stringify({ paneKey: getPaneKey() }));
  // The receipt is its own JSON line: the pin hook validates an exact
  // schema, so the receipt must never be merged into other output.
  console.log(JSON.stringify(createSelectionReceipt({
    projectName,
