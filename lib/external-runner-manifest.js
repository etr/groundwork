...
function scanRequireCalls(source, state) {
  ...
  // Indirect require references: `const r = require; r(...)`. An alias
  // makes the eventual call a require call — tracked, never silently missed.
  const aliases = new Set();
  for (let i = 0; i + 3 < tokens.length; i++) {
    const candidate = tokens[i];
    const assignment = tokens[i + 1];
    const required = tokens[i + 2];
    const boundary = tokens[i + 3];
    if (candidate.type !== 'id' || candidate.value === 'require') continue;
    if (assignment.type !== 'punct' || assignment.value !== '=') continue;
    if (required.type !== 'keyword' || required.value !== 'require') continue;
    if (boundary.type !== 'punct' || ![';', ',', ')', ']', '}'].includes(boundary.value)) continue;
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    const isRequire = (token.type === 'keyword' && token.value === 'require')
      || (token.type === 'id' && aliases.has(token.value));
