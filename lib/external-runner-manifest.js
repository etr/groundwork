...
function tokenizeJavaScript(source) {
  ...
  while (i < source.length) {
    ...
    // Template literals (flag ${..} substitution)
    if (ch === '`') {
      const start = i;
      i++;
      let depth = 0;
      ...
      i++; // closing backtick
      push('template', body, start, i, { hasSubstitution: /\$\{/.test(body) });
      previousMeaningful = tokens[tokens.length - 1];
      continue;
    }
    ...
    if (/[0-9.]/.test(ch)) {
      const start = i;
      while (i < source.length && /[0-9a-fA-FxXoObBeE_.+-]/.test(source[i]) && !/[+-]$/.test(source.slice(start, i + 1))) i++;
...
function scanRequireCalls(source) {
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    // Skip wrapping close-parens: `(require)(...)` / `(import)(...)`.
    let cursor = index + 1;
    while (cursor < tokens.length && tokens[cursor].type === 'punct' && tokens[cursor].value === ')') cursor++;
    if (cursor >= tokens.length || tokens[cursor].type !== 'punct' || tokens[cursor].value !== '(') continue;
    if (isImport && cursor !== index + 1) continue; // `(import)(x)` is not a dynamic import
...
function scanModuleClosure(entry, declaredSources, problems) {
  ...
  for (const call of scanRequireCalls(text)) {
    ...
    // classification marker on the physical line span of the call.
