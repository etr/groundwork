...
- Only findings owned by \`request-changes\` reviews enter a fixer batch. Approved major findings remain unworked findings; major severity alone never overrides the reviewer's verdict. An approved critical finding is an invalid reviewer artifact.
- Before every reviewer or fixer spawn, set \`fork_turns="none"\`. Reviewer prompts may contain only the frozen baseline, task definition, changed paths/diff stat, relevant spec/test paths, coordinator-assigned artifact paths, prior finding IDs/status and normalized closure records, the validated fixer result, files touched, current verification evidence, the closure brief, and scoped evidence. Fixer prompts may contain only the working directory, iteration, \`findings_dir\`, manifest/result basenames, validator path, the coordinator-authored repair envelope, and the fixed execution contract; never include raw conversation history.
...
- Use Terra/high for the validation coordinator throughout the validation run.
`;
...
- Before a focused re-review, provide a closure brief containing prior finding records and status, validated semantic repair claims, \`files_touched\`, the repair delta, disturbed cleared invariants, current verification evidence, verify-only questions, closed scope, and the stop condition. Do not replay conversation history or run an unrelated full reviewer suite.
...
const CODEX_FIXER_STEPS = `2. **Adjudicate and Spawn Fix Agent** — Reminder: do not Edit/Write source files; dispatch every fix through an isolated fixer.
   ...
   Validate the coordinator-owned manifest with \`--check-findings\`. Its \`finding_ids\` list is the complete authorized fix scope: critical/major findings from \`request-changes\` reviews. Approved findings never enter this list.
...
4. **Run Causal Closure Review** — Run every required post-fix project gate on the current worktree as specified by the source workflow. Once they pass, update the coordinator state and checkpoint \`fixer-result-ready -> gates-complete\`. Then bump \`iteration_number\`, set \`review_mode: closure-review\`, and write a coordinator-authored closure brief for each impacted reviewer containing prior closure records, semantic repair claims, the repair delta, disturbed invariants, verify-only questions, closed scope, the causal blocking rule, and the immediate approval stop condition.
...
1. Spawn a default subagent with \`fork_turns="none"\`, model \`gpt-5.6-terra\` at \`high\` effort, and description \`Validate [TASK-NNN]\`.
...
function applyValidatePolicy(text) {
  ...
  if (![fixerStart, fixerEnd, reviewLocation, semanticAggregation, semanticTracking]
