...
const VALIDATION_POLICY = `## Codex Validation Budget

This section overrides conflicting retry or model guidance below.

\`max_validation_iterations = 3\` (one comprehensive pass plus at most two targeted rechecks).
...
- If iteration 3 still returns a blocking finding, do not start another fixer. Persist unresolved findings, clean up the temporary directory, and return \`Validation INCOMPLETE (iteration cap reached)\` to the caller.
...
function applyValidatePolicy(text) {
  const retryRule = '**Rule**: You MUST continue this loop until ALL agents return `approve`. No exceptions. No user overrides.';
  ...
  if (!text.includes(retryRule) || !text.includes(targetedRule)) {
  ...
  return prepend(
    ...
    text
      .replace(
        retryRule,
        '**Rule**: Continue until all agents approve or `max_validation_iterations` is reached.'
      )
      .replace(
        targetedRule,
