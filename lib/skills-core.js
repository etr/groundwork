...
const { getSpecsDir, getProjectRoot, getProjectName } = require('./project-context');
 ...
 * Replaces {{specs_dir}}, {{project_root}}, {{project_name}} with
 * values from project-context.js. All bindings follow the shared
 * absolute-path contract (invariant to the caller's working directory).
 *
 * @param {string} content - Skill content with template variables
...
function resolveTemplateVariables(content) {
    ...
    return content
        .replace(/\{\{specs_dir\}\}/g, specsDir)
        .replace(/\{\{project_root\}\}/g, projectRoot)
        .replace(/\{\{project_name\}\}/g, projectName)
        .replace(/\{\{effort_level\}\}/g, effortLevel);
}

/**
 * Read the current effort level from Claude Code settings.
 *
 * @returns {string} Effort level: "max", "high", or "default"
 */
