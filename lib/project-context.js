 ...
 *
 * Session isolation: each terminal gets its own session file keyed by TTY hash,
 * so multiple Claude instances in the same monorepo don't overwrite each other.
 *
 * Resolution chain:
...
const SESSIONS_DIR = path.join(process.env.HOME || '', '.claude', 'groundwork-state', 'sessions');
...
/**
 * Compute a session ID from a TTY device string.
 * Uses the GROUNDWORK_SESSION_TTY env var if available.
 *
 * @param {string} [tty] - TTY device string (e.g., "pts/0"). Uses env var if omitted.
 * @returns {string|null} 12-char hex hash, or null if no TTY available
 */
function getSessionId(tty) {
  const sessionTty = tty || process.env.GROUNDWORK_SESSION_TTY;
  if (!sessionTty) return null;
  return crypto.createHash('md5').update(sessionTty).digest('hex').substring(0, 12);
}
...
function persistSessionSelection(sessionId, projectName, projectPath) {
  try {
    ...
    const data = {
      ...
      tty: process.env.GROUNDWORK_SESSION_TTY || ''
    };
 ...
 * 1. Session file for current TTY
...
function restoreSelection(sessionId) {
  ...
  if (recentResult) {
    // If we have a session ID, persist to create a session file for this TTY
    if (sid) {
