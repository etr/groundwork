 ...
 * <project root>/.debug/ — mirroring getPlansDir()'s scoping and relativity.
 *
 * @returns {string} Path to the debug dir (e.g., ".debug" or "apps/web/.debug")
 */
function getDebugDir() {
  return path.join(path.dirname(getSpecsDir()), '.debug');
}
 ...
 * slugs, so they live at <project root>/.architecture/ — mirroring
 * getPlansDir()'s scoping and relativity.
 *
 * @returns {string} Path to the research dir (e.g., ".architecture" or "apps/web/.architecture")
 */
function getResearchDir() {
  return path.join(path.dirname(getSpecsDir()), '.architecture');
}
