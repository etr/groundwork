 ...
 *
 * Pane isolation: each terminal pane gets its own state file keyed by stable
 * environment identity when available, then by TTY ancestry. Pane keys survive
 * /clear and cwd changes while isolating concurrent panes. The session_id is
 * recorded as audit metadata only — never used as the lookup key.
 *
 * Resolution chain:
 ...
 * found, then normalize the result (e.g. /dev/pts/3 → pts_3).
 *
 * Fallback chain when no TTY is available (headless, detached, nohup):
 * 1. tmux-* from stable environment identity
 * 2. pts_* from process walk
 * 3. cwd-<sha1(absolute cwd)[:12]> (deterministic per working directory)
