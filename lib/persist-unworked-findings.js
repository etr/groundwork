 ...
 * without it, `<YYYY-MM-DD>_<HHMMSS>_<slug>.md` with a `-N` uniquifier when
 * a same-second collision exists. The write always happens and `status` is
 * always truthful.
 *
...
function main() {
  ...
  if (runId) {
    ...
    // Timestamped fallback (manual invocations): second-granularity keys can
    // collide, so uniquify instead of silently skipping.
    const base = `${ts.datePart}_${ts.timePart}_${slug}`;
    let candidate = path.join(outDir, `${base}.md`);
    let suffix = 2;
    while (fs.existsSync(candidate)) {
      candidate = path.join(outDir, `${base}-${suffix++}.md`);
    }
    outPath = candidate;
  }
