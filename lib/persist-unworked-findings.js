...
// Legacy records carry no disposition field. They are persisted only when
// demonstrably actionable: a record whose recommendation is an explicit
// "None…" or whose body reads as a closure re-check (resolved/already
// addressed/approved) is closed work, not an open item.
const LEGACY_CLOSED_TEXT = /is RESOLVED|already addressed|approved as claimed|resolved as claimed/i;
const LEGACY_NONE_RECOMMENDATION = /^none\b/i;

function legacyRecordIsDemonstrablyActionable(record) {
  if (LEGACY_NONE_RECOMMENDATION.test(String(record.recommendation || '').trim())) return false;
  if (LEGACY_CLOSED_TEXT.test(String(record.finding || ''))) return false;
  if (LEGACY_CLOSED_TEXT.test(String(record.recommendation || ''))) return false;
  return true;
}
...
function shouldPersist(record, fixedIds) {
  ...
  return legacyRecordIsDemonstrablyActionable(record);
}
