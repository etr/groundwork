...
// Tier tokens are matched in every export target: the Codex builtin names and
// the Claude source names (still present in claude-code/OpenCode/Kiro/Pi
// exports) plus the ZCode GLM family names (ZCode exports replace Claude
// names with GLM family wording before the override runs).
const TIER_ALIASES = {
  light: ['Luna', 'luna', 'Haiku', 'haiku'],
  balanced: ['Terra', 'terra', 'Sonnet', 'sonnet'],
  deep: ['Sol', 'sol', 'Opus (1M context)', 'opus[1m]', 'Opus', 'opus'],
};
const ZCODE_TIER_ALIASES = {
  light: ['GLM-Flash'],
  deep: ['GLM'],
};
const TIER_TOKENS = {};
...
function tierAliasesFor(harness) {
  const aliases = {};
  for (const tier of TIERS) aliases[tier] = [...TIER_TOKENS[tier]];
  if (harness === 'zcode' || harness === 'zcode-plugin') {
    for (const tier of TIERS) aliases[tier].push(...(ZCODE_TIER_ALIASES[tier] || []));
  }
  return aliases;
}

// Longest tokens first so overlapping names replace atomically: `opus[1m]`
// before `opus`, `GLM-Flash` before `GLM`, `gpt-5.6-sol` before `sol`.
function tierMatchPlan(harness) {
  const plan = [];
  for (const [tier, tokens] of Object.entries(tierAliasesFor(harness))) {
    for (const token of tokens) plan.push({ tier, token });
  }
  return plan.sort((left, right) => right.token.length - left.token.length);
}

// Lookarounds instead of \b so tokens that end in non-word characters
// (opus[1m], GLM-Flash) still anchor without disabling their trailing edge.
function tokenPattern(token, suffix) {
  return `(?<![A-Za-z0-9_])${escapeRegExp(token)}${suffix}(?![A-Za-z0-9_])`;
}

function replaceTierTokens(text, replacements, effort, harness) {
  let result = text;
  for (const { tier, token } of tierMatchPlan(harness)) {
    const replacement = replacements[tier];
    if (effort) {
      const combined = new RegExp(`${tokenPattern(token, '')}\\/(?:${EFFORTS.join('|')})(?![A-Za-z0-9_])`, 'g');
      result = result.replace(combined, () => `${replacement}/${effort}`);
    }
    const bare = new RegExp(tokenPattern(token, ''), 'g');
    result = result.replace(bare, () => replacement);
  }
  return result;
}
...
function replacePolicyEfforts(text, effort) {
  ...
  return text
    ...
    .replace(/reasoning at (low|medium|high|max)\b/g, `reasoning at ${effort}`)
    .replace(/reasoning effort to (low|medium|high|max)\b/g, `reasoning effort to ${effort}`);
}

function applyToText(override, kind, name, text, harness) {
  const exactMap = kind === 'skill' ? override.skills : override.agents;
  ...
  const result = replaceTierTokens(text, replacementsFor(override, kind, name), override.effort, harness);
...
function main(argv) {
  ...
  if (command === 'transform') {
    ...
    process.stdin.on('end', () => {
      process.stdout.write(applyToText(override, args.kind, args.name, input, args.harness));
    });
    return;
  }
