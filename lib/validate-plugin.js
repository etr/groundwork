...
function checkFile(filePath) {
  ...
  if (filePath.includes('/skills/')) {
    // Skills are both user-invokable (slash) and model-invokable, so they accept the
    // invocation-control and command-style fields that used to live on commands.
    const allowedFields = ['name', 'description', 'requires', 'user-invocable',
  ...
  // Check for non-empty allowed-tools when declared, and argument-hint when args are used.
  // These fields now live on skills (commands were merged into skills).
  if (filePath.includes('/skills/')) {
    ...
    // Check for missing argument-hint when the skill accepts arguments
    ...
    if (usesArguments && !hasArgumentHint) {
      addIssue(filePath, 'Skill accepts arguments but missing argument-hint in frontmatter', 'warning');
    }
  }
  ...
  // Check that ${CLAUDE_PLUGIN_ROOT}/<path> references resolve to a real file or directory.
  // These are runtime resource pointers (references, templates, lib scripts). A broken one
  // fails silently at runtime — the sibling-file check below does NOT cover them — so a
  // missing target is an error.
  const pluginRootRefPattern = /\$\{CLAUDE_PLUGIN_ROOT\}\/([A-Za-z0-9._/-]+)/g;
  const seenPluginRootRefs = new Set();
  let pluginRootMatch;
  while ((pluginRootMatch = pluginRootRefPattern.exec(content)) !== null) {
    // Strip trailing sentence punctuation that isn't part of a path (e.g. a closing period).
    const refPath = pluginRootMatch[1].replace(/[.,;:)]+$/, '');
    if (!refPath || seenPluginRootRefs.has(refPath)) continue;
    seenPluginRootRefs.add(refPath);
    if (!fs.existsSync(path.join(PLUGIN_ROOT, refPath))) {
      addIssue(filePath, `\${CLAUDE_PLUGIN_ROOT} reference not found: ${refPath}`);
    }
  }
}
...
function checkDirectory(dir, pattern) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // For agents, check .md files directly
