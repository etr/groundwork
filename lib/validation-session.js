...
const { writeFileSyncAtomic, writeJsonSyncAtomic } = require('./atomic-write');
...
function acquireOwnedLockWithRetry(lockFile) {
  ...
  // Exponential backoff (10ms→250ms cap), mirroring waitForMutationEntry in
  // lib/lease-mutation.js: each contention retry costs a full fsync-heavy
  // mutation turn plus a holder-liveness probe, so retries space out and the
  // probe runs at most once per backoff interval instead of every 25ms. The
  // deadline window itself is unchanged.
  let retryDelay = 10;
  for (;;) {
    try {
...
function sessionBusyMessage(state, identity) {
  ...
  return (
    `a validation session for ${identity.taskId} on branch ${identity.branch} is already active` +
      ...
      ` ${state.runId} --capability-file <capability_file> (the open result returned` +
      ' a capability_file path when it created, resumed, or reclaimed the session).' +
...
// Capability transport (file form): the bearer capability lives in an
// owner-only (0600) file the caller controls — typically saved directly by
// `open --save-capability`. This is the manual-session counterpart of the
// runner's private store: the raw token never touches argv, the environment,
// the transcript, or durable session state (which keeps only its digest).
function capabilityFromFile(file) {
  requireString(file, 'capabilityFile');
  // One O_NOFOLLOW descriptor, then fstat: a symlink is rejected by open
  // itself and the permission check races with nothing — the stat describes
  // exactly the descriptor being read.
  let fd;
  try {
    fd = fs.openSync(file, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
  } catch (error) {
    if (error.code === 'ELOOP') {
      throw new Error(`the capability file is a symlink and will not be read: ${file}`);
    }
    throw new Error(`the capability file cannot be read: ${file}`);
  }
  try {
    const stat = fs.fstatSync(fd);
    if (!stat.isFile()) throw new Error(`the capability file is not a regular file: ${file}`);
    if ((stat.mode & 0o077) !== 0) {
      throw new Error(`refusing to read a group- or world-readable capability file: ${file} (chmod 600 it first)`);
    }
    const content = fs.readFileSync(fd, 'utf8').trim();
    if (!content) throw new Error(`the capability file is empty: ${file}`);
    // Two sanctioned on-disk shapes: a raw single-line token (the manual
    // capability file) or the runner store JSON ({version, runId, ownerToken,
    // epoch}). Both are owner-only files beside the session slot.
    if (content.startsWith('{')) {
      let parsed;
      try {
        parsed = JSON.parse(content);
      } catch {
        throw new Error(`the capability file is malformed JSON: ${file}`);
      }
      if (!parsed || typeof parsed.ownerToken !== 'string' || !parsed.ownerToken) {
        throw new Error(`the capability file does not carry an ownerToken: ${file}`);
      }
      return parsed.ownerToken;
    }
    const firstLine = content.split(/\r?\n/, 1)[0].trim();
    if (!firstLine) throw new Error(`the capability file is empty: ${file}`);
    return firstLine;
  } finally {
    fs.closeSync(fd);
  }
...
function saveManualCapability(file, ownerToken) {
  requireString(file, 'saveCapability');
  // Single-line raw capability (not JSON): capabilityFromFile reads exactly
  // one trimmed line, so the store and the loader share one format.
  writeFileSyncAtomic(file, `${ownerToken}\n`);
  // Tighten on every replacement, mirroring the runner store.
  fs.chmodSync(file, 0o600);
}
...
function cliResult(opened) {
  return {
    ...
    // Bearer capability transport: manual opens persist the raw token to an
    // owner-only file and return its NON-SECRET path here; the raw token
    // itself appears only when no file could be persisted (runner mode uses
    // its own private store and never echoes a token).
    owner_token: opened.ownerToken || null,
    capability_file: opened.capabilityFile || null,
  };
}
...
function main(argv = process.argv.slice(2)) {
  ...
  let result;
  const capability = args.capabilityFile
    ? capabilityFromFile(args.capabilityFile)
    : capabilityFromStdin(command !== 'open' && command !== 'abandon');
  if (command === 'open') {
    // `capability` was read exactly once above (file or stdin) — never read
    // stdin a second time here: the first read already consumed the pipe.
    const opened = openValidationSession({
      ...
      ownerToken: capability,
    });
    let capabilityFile = null;
    if (opened.ownerToken) {
      if (args.runnerMode) {
        // The module already persisted the runner's private store; expose
        // only its NON-SECRET path. The raw token never reaches stdout in
        // any mode.
        capabilityFile = runnerCapabilityPath(path.dirname(opened.runDir));
      } else {
        // Manual sessions persist their capability at a deterministic,
        // caller-visible location beside the run directory (mirroring the
        // runner's private store). The stdout result then carries the
        // NON-SECRET file path instead of the raw token.
        capabilityFile = args.saveCapability
          || path.join(path.dirname(opened.runDir), `manual-capability-${opened.state.runId}.json`);
        saveManualCapability(capabilityFile, opened.ownerToken);
      }
      opened.ownerToken = null;
    }
    opened.capabilityFile = capabilityFile;
    result = cliResult(opened);
  } else if (command === 'checkpoint') {
    const state = checkpointValidationSession(args.runDir, { ...args, ownerToken: capability });
    ...
    const state = beginFixerTransaction(args.runDir, { ...args, ownerToken: capability });
    ...
    const state = completeFixerTransaction(args.runDir, { ...args, ownerToken: capability });
    result = { status: 'fixer-completed', stage: state.stage, iteration: state.iteration, revision: state.revision };
  } else if (command === 'complete') {
    const state = completeValidationSession(args.runDir, { ...args, ownerToken: capability });
    ...
    heartbeatLoop(requireString(args.runDir, 'runDir'), requireString(capability, 'ownerToken'));
    result = { status: 'heartbeat-stopped' };
  } else if (command === 'heartbeat-stop') {
    const state = heartbeatStop(requireString(args.runDir, 'runDir'), requireString(capability, 'ownerToken'));
    result = { status: 'heartbeat-stopped', revision: state.revision };
  } else if (command === 'heartbeat-beat') {
    const state = heartbeatBeat(requireString(args.runDir, 'runDir'), requireString(capability, 'ownerToken'));
