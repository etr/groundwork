...
function acquireOwnedLockWithRetry(lockFile) {
  ...
  for (;;) {
    try {
      ...
      if (!/another process holds|could not acquire/.test(error.message) || Date.now() >= deadline) throw error;
      sleepSync(25);
    }
  }
}
...
function acquireOwnedLockForHeartbeat(runDir, lockFile, ownerToken) {
  ...
  for (;;) {
    try {
      ...
      const contention = /another process holds|could not acquire/.test(error.message)
