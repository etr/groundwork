...
function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'open') {
    const opened = openValidationSession({
      ...
      ownerToken: args.capabilityFile ? capability : capabilityFromStdin(false),
    });
    if (args.saveCapability && opened.ownerToken) {
      // The capability is persisted owner-only; the stdout result then
      // omits the raw token entirely (cliResult drops it below).
      saveManualCapability(args.saveCapability, opened.ownerToken);
