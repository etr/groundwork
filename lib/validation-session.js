...
const { acquireOwnedLock, unlinkIfSameInode } = require('./owned-lock');
...
function tryAcquireOpenLock(parent, lockFile, kill, hooks = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (descriptor !== undefined) {
      ...
      return () => releaseOpenLockVerified(lockFile, createdStat);
    }
...
/**
 * Release the open lock only when the lockfile still is the exact inode this
 * process created and still records this process as its holder: a successor
 * that re-acquired the slot after reaping a stale us is never unlinked.
 */
function releaseOpenLockVerified(lockFile, createdStat) {
  let current;
  try {
  ...
  if (current.dev !== createdStat.dev || current.ino !== createdStat.ino) return;
  try {
    const lock = JSON.parse(fs.readFileSync(lockFile, 'utf8'));
    if (lock.pid !== process.pid || lock.host !== os.hostname()) return;
  } catch {
    return; // Unreadable: not verifiably ours — leave it alone.
  }
  try {
