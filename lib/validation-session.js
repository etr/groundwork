 ...
 *              staleness window; only the capability owner may mutate.
...
function sessionLiveness(state, now = Date.now()) {
  ...
  if (heartbeat.registered) {
    ...
    if (Number.isFinite(lastBeat) && now - lastBeat < sessionStaleMs()) return 'live';
    return 'stale';
  }
...
function withRunLock(runDir, ownerToken, mutate) {
  const initial = loadSession(runDir);
...
function acquireOwnedLockWithRetry(lockFile) {
  ...
  for (;;) {
    try {
      ...
      if (!/another process holds/.test(error.message) || Date.now() >= deadline) throw error;
      sleepSync(25);
    }
  }
}
...
function sessionBusyMessage(state, identity) {
  ...
  return (
    `a validation session for ${identity.taskId} on branch ${identity.branch} is already active` +
      ...
      ` ${state.runId} --owner-token <capability> (both values were returned when the session` +
...
function saveRunnerCapability(parent, runId, ownerToken, epoch) {
  ...
  fs.writeFileSync(file, `${JSON.stringify({ version: 1, runId, ownerToken, epoch }, null, 2)}\n`, {
    mode: 0o600,
  });
  if (!existed) {
    try {
      ...
      // Best effort — the file was created with 0o600 already.
    }
  }
}
...
function recoverOrResume(state, runDir, ownerToken) {
  if (state.stage === 'fixer-inflight') {
    const now = new Date().toISOString();
    const recovered = { ...state, stage: 'fixer-prepared', revision: state.revision + 1, updatedAt: now };
  ...
  return { status: 'resumed', runDir, findingsDir: runDir, state, ownerToken };
}
...
function openLockedSession(identity, parent, activeFile, input) {
  ...
  if (fs.existsSync(activeFile)) {
    ...
    if (liveness === 'live' || liveness === 'grace') {
      ...
      atomicWriteJson(path.join(runDir, '.validation-session.json'), resumed);
      if (input.runnerMode) saveRunnerCapability(parent, resumed.runId, token, resumed.owner.epoch);
      return recoverOrResume(resumed, runDir, token);
    }

    // Stale owner: safe takeover, revalidated under the slot lock this open
    // already holds. The successor epoch invalidates the old capability.
    const now = Date.now();
    const minted = mintOwner(state.owner, now);
    let next = {
      ...state,
      owner: minted.owner,
      revision: state.revision + 1,
    ...
    let recovery = null;
    if (next.stage === 'fixer-inflight') {
      next = { ...next, stage: 'fixer-prepared' };
      recovery = { action: 'rerun-fixer' };
    }
    atomicWriteJson(path.join(runDir, '.validation-session.json'), next);
    if (input.runnerMode) saveRunnerCapability(parent, next.runId, minted.token, minted.owner.epoch);
    return {
      status: 'reclaimed',
      runDir,
      findingsDir: runDir,
...
function heartbeatRegister(runDir, ownerToken) {
  return withRunLock(runDir, ownerToken, (loaded) => {
...
function heartbeatBeat(runDir, ownerToken) {
  return withRunLock(runDir, ownerToken, (loaded) => {
    const heartbeat = loaded.state.owner.heartbeat;
    if (loaded.state.owner.terminal) throw new Error('heartbeat worker exiting: validation session is complete');
    if (heartbeat.stopped) throw new Error('heartbeat worker exiting: explicitly stopped');
    return {
      ...loaded.state,
      owner: {
        ...loaded.state.owner,
...
function heartbeatLoop(runDir, ownerToken) {
  ...
  for (;;) {
    ...
    try {
      ...
      if (/exiting:/.test(error.message)) return 'stopped';
      throw error;
    }
  }
}
...
function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'open') {
    ...
    heartbeatLoop(requireString(args.runDir, 'runDir'), requireString(args.ownerToken, 'ownerToken'));
    result = { status: 'heartbeat-stopped' };
  } else if (command === 'heartbeat-stop') {
    const state = heartbeatStop(requireString(args.runDir, 'runDir'), requireString(args.ownerToken, 'ownerToken'));
    result = { status: 'heartbeat-stopped', revision: state.revision };
  } else if (command === 'heartbeat-beat') {
    const state = heartbeatBeat(requireString(args.runDir, 'runDir'), requireString(args.ownerToken, 'ownerToken'));
