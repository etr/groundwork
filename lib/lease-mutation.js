...
// Queue staging records are queue entries in transit: liveness follows the
// recorded (pid, processStart) pair, with an age fallback for records that
// cannot be parsed or probed.
function reclaimStaleStagingEntries(directory, dependencies = {}) {
  ...
  for (const name of fs.readdirSync(directory)) {
    ...
    try {
      ...
      if (record && record.token === match[1] && Number.isInteger(record.pid) && record.pid >= 1
          && typeof record.processStart === 'string' && record.processStart) {
        stale = processStartIdentity(record.pid, dependencies) !== record.processStart;
      } else {
...
function waitForLeaseMutation(dependencies, milliseconds) {
