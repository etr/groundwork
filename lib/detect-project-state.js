...
try {
  ...
  // An invalid .groundwork.yml must never degrade to single-project output:
  // report the typed config error and exit non-zero so callers (hooks) can
  // surface it instead of silently proceeding on wrong bindings.
  if (error && error.name === 'ProjectConfigError') {
    console.error(`detect-project-state: ${error.message}`);
    console.log(JSON.stringify({ configError: error.code, message: error.message }));
    process.exit(1);
  }
