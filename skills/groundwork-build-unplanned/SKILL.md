...
description: Build a feature from description with worktree isolation and TDD.
---
...
- **No argument:** Use `Ask the user` to ask:
  > "What feature would you like to build? Describe it briefly."

## Step 2: Plan → `plan-task`

Call `the groundwork-plan-task workflow` with the blurb from Step 1 in the conversation context. Do NOT pass a task_id — this triggers blurb/feature mode in plan-task, which will:
...
Call `the groundwork-implement-task workflow` with `plan_file_path` in the conversation context. Do NOT pass a task_id.
