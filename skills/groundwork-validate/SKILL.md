...
If arguments include `--noninteractive`, skip the model recommendation and never call `Ask the user`. Preserve normal non-runner result and commit behavior. When user input would otherwise be required, persist unexecuted findings, preserve the durable session, and return `Validation INCOMPLETE ({N} iter, reason: <reason>)`. Do not restart validation.

## Runner Mode

If session context contains `GROUNDWORK_RUNNER_MODE=true`:

- Skip the model recommendation pre-flight and do not call `Ask the user`.
...
Unless `--noninteractive` is set, use `Ask the user`:

```json
{
  "questions": [{
    "question": "Do you want to switch? Fix loop management and domain spillover judgment across 9 agents benefits from consistent reasoning.\n\nTo switch: cancel, raise the reasoning effort to max in the model settings (and switch to GLM in the model picker if you are on GLM-Flash), then re-invoke this skill.",
    "header": "Recommended: GLM with reasoning at max",
    "options": [
      { "label": "Continue" },
...
You MUST NOT modify source files yourself during this loop. All fixes go through the `validation-fixer` sub-task. Allowed orchestrator writes: coordinator-owned artifacts inside the durable `findings_dir`, state transitions through `lib/validation-session.js`, and `lib/persist-unworked-findings.js` in step 5.5. Never edit helper-owned `.validation-session.json` or `active.json` directly. Zero `Edit`/`Write`/`NotebookEdit`/`sed -i`/`tee`/redirect calls on anything else. In runner mode, do not stage or commit after step 5.5; the runner records the approved tree. No exceptions — even one-character cosmetic fixes go through the sub-task. **Why:** this skill exists to keep fix work out of the orchestrator's context window; direct edits burn the budget saved by dispatching the reviewers and pollute the next iteration's context with diff details.

## Validation State Model

Read `<skill-directory>/references/validation-review-protocol.md` and `<skill-directory>/references/validation-session-protocol.md`. Preserve one frozen validation baseline through the run and across process restarts. The baseline freezes required behavior and review scope, not implementation shape.
...
2. **CWD mismatch check (monorepo only):**
   ...
   - If CWD is inside a different project's path → with `--noninteractive`, keep the explicitly selected project and continue; otherwise warn via `Ask the user`:
     ...
     If the user switches, Follow the groundwork-select-project workflow steps.
...
```bash
node <skill-directory>/scripts/validation-session.js open \
  --repo-root "<repo_root>" \
  ...
  <optional --resume-run "<run_id>" --capability-file "<capability_file>" — resume from the capability file a prior open persisted (the non-secret `capability_file` path in its result). Include both on every re-open within this validation. The capability never appears on the command line, in the environment, in the transcript, or in session state. In runner mode, the phase prompt supplies RESUME VALIDATION SESSION=<run_id> (environment GROUNDWORK_VALIDATION_RUN_ID): pass that value here verbatim; the runner's retained capability is applied automatically>
```

Parse only the returned one-line JSON. Save `run_id`, `run_dir`, `findings_dir`, `stage`, `iteration`, `coordinator_file`, and `capability_file` (the non-secret path to the persisted capability, returned by `created`, `resumed`, and `reclaimed`; there is never an `owner_token` to save — the raw token exists only inside its 0600 file).

**Capability handling:** every `open` persists the bearer capability itself to an owner-only (0600) file beside the run directory — `manual-capability-<run_id>.json` for manual runs, the runner's `runner-capability.json` in runner mode — and returns its NON-SECRET path as `capability_file`. The raw token never appears on stdout, argv, the environment, the transcript, or durable session state (which stores only its digest); both file shapes are accepted by `--capability-file`. Save that literal `capability_file` path and pass `--capability-file "<capability_file>"` to every mutating command, including heartbeat-loop/stop and every re-open with `--resume-run` (a resumed or reclaimed open refreshes the same file and returns it again). If the file is lost, the session cannot be mutated until its heartbeat is provably stale; re-open then to reclaim a successor capability.

**Heartbeat worker (start immediately after a successful open):** the session's liveness is this worker, not your process. Run it in the background for the whole validation and stop it in every exit path (completion, failure, or trap) — both commands authenticate through the capability file returned by `open`:

```bash
node <skill-directory>/scripts/validation-session.js heartbeat-loop \
  --run-dir "<run_dir>" --capability-file "<capability_file>" &
# on completion or failure:
node <skill-directory>/scripts/validation-session.js heartbeat-stop \
  --run-dir "<run_dir>" --capability-file "<capability_file>"
```
...
| Agent (`sub-task_type`) | Context to Provide |
|-------------------------|-------------------|
| `the code-quality-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `test_file_paths` |
| `the test-quality-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `test_file_paths` |
| `the security-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition` |
| `the spec-alignment-checker agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `specs_path` |
| `the architecture-alignment-checker agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `architecture_path` |
| `the code-simplifier agent` | `changed_file_paths`, `diff_stat`, `task_definition` |
| `the housekeeper agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `task_status`, `specs_path`, `architecture_path`, `design_system_path` |
| `the performance-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition` |
| `the design-consistency-checker agent` | `changed_file_paths`, `diff_stat`, `design_system_path` |
| `the cloud-infrastructure-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition`, `architecture_path` |
| `the conventions-reviewer agent` | `changed_file_paths`, `diff_stat`, `task_definition` |
...
```bash
node <skill-directory>/scripts/validation-session.js checkpoint \
  --run-dir "<run_dir>" \
...
2. **Adjudicate and Spawn Fix Agent** — Build a repair envelope for every blocking closure record before spawning the fixer:
   ...
   ```bash
   node <skill-directory>/scripts/validation-session.js begin-fixer \
     --run-dir "<run_dir>" \
     --iteration "<N>" \
...
> Spawn the `validation-fixer` agent for **Fix validation findings (iteration N)** with this task:
>
...
3. **Parse Fix Agent Result** — require global IDs plus semantic repair claims (`root_cause`, `change`, `evidence`, `contracts_changed`):
   ...
   - `RESULT: FAILURE | [reason]` → log the failure reason; in runner mode, return `RESULT: FAILURE` immediately; with `--noninteractive`, return `Validation INCOMPLETE`; otherwise escalate to user via `Ask the user`
   ...
   ```bash
   node <skill-directory>/scripts/validation-session.js complete-fixer \
     --run-dir "<run_dir>" \
     --iteration "<N>" \
...
- Outside runner and noninteractive modes, use Ask the user for guidance
...
3. Run:
   ```bash
   node <skill-directory>/scripts/persist-unworked-findings.js \
...
```bash
node <skill-directory>/scripts/validation-session.js complete \
  --run-dir "<run_dir>" \
