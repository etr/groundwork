...
const { redactCredentials } = require('./redact');
...
function shouldPersist(record, fixedIds) {
  ...
  // Authoritative signal only: the explicit disposition field (or a verified
  // fixed ID). Loose text heuristics over finding/recommendation prose are
  // deliberately NOT consulted — prose is not a disposition.
  const disposition = record.disposition;
  if (disposition !== undefined && disposition !== null && disposition !== '') {
    const normalized = String(disposition).toLowerCase();
    if (CLOSED_DISPOSITIONS.has(normalized)) return false;
    if (normalized !== 'actionable') {
      // Fail closed: an unrecognized value means the producer/consumer
      // disposition protocol has drifted, and silently dropping or keeping
      // the finding would corrupt the durable ledger either way.
      throw new Error(
        `unrecognized disposition "${disposition}" on ${record.globalId}: `
        + `expected "actionable", one of ${[...CLOSED_DISPOSITIONS].join('|')}, or no disposition field`
      );
    }
    return true;
  }
  // Legacy records with no disposition field are treated as actionable:
  // migrate them by stamping an explicit disposition (or supplying verified
  // fixed IDs) — do not infer one from their wording.
  return true;
}
...
function renderMarkdown({ findings, taskId, humanTimestamp, counts }) {
  ...
  for (const [severityKey, heading] of SEVERITY_HEADINGS) {
    ...
    for (const f of bucket) {
      ...
      // Reviewers quote the exact secret-bearing code they flag; the ledger
      // is a committed artifact, so finding text is redacted before render.
      if (f.finding) lines.push(`   ${redactCredentials(f.finding)}`);
      if (f.recommendation) lines.push(`   *Recommendation:* ${redactCredentials(f.recommendation)}`);
      lines.push('');
      counter++;
    }
  }
