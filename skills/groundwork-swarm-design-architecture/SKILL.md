---
name: groundwork-swarm-design-architecture
description: Design architecture using parallel advocate agents for adversarial technology comparison.
---
...
LOAD CONTEXT → IDENTIFY DECISIONS → RESEARCH SWARM → ITERATE DECISIONS → DOCUMENT → NEXT STEP
      ...
      └────────────────┴───────────────────┴──────────────────┴──────────────┴───────────┘
                                           │
                                   RESEARCH JOURNAL
                              {{research_dir}}/{slug}-xxxxxx/research.md
```
...
Follow the `groundwork-design-architecture` skill's Step 1 exactly.
...
For each swarm-worthy decision area, spawn one advocate per technology option (2-4 advocates per decision area). Each advocate is an Agent call using the `the researcher agent` agent with an advocate prompt overlay.
...
Write findings to an invocation-unique research journal — `RESEARCH_PATH="$(mktemp -d "{{research_dir}}/${SLUG}-XXXXXX")/research.md"` — an atomically unique `mktemp` run directory keyed on the feature slug. This survives context management and documents the research for future reference; concurrent research on the same slug can never overwrite these journals.
...
3. Follow the groundwork-design-architecture workflow steps and continue with its standard Step 3
