...
- If the user wants to switch, Follow the groundwork-select-project workflow steps and then continue
...
Follow the workflow steps in skill `groundwork-understanding-feature-requests` to clarify the feature and check for contradictions.
...
1. **Check if PRD exists** - Look for single file first, then directory
   - If missing, create `{{specs_dir}}/product_specs.md` using the template in `<skill-directory>/references/product-design/prd-template.md`
...
- Follow the groundwork-split-specs workflow steps — this runs automatically with no user input needed.
...
After successfully updating the PRD, use the `Ask the user` tool to present the next step options:
...
- **Create tasks**: Run `/groundwork-create-tasks` to generate implementation tasks for the new feature
...
- `<skill-directory>/references/product-design/ears-examples.md` - Curated examples from various domains
- `<skill-directory>/references/product-design/prd-template.md` - Template for creating new PRD files
