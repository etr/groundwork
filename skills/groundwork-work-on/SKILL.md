...
2. After Step 2 produces the plan file, write the plan mode output:
   ...
   ### Execution Context
   **Working Directory:** [run `node <skill-directory>/scripts/worktree-identity.js <TASK-NNN>` and use its `path` value — project-qualified in monorepos]
...
3. Call `Proceed with implementation`
...
- All `Ask the user` calls are skipped
- On completion or failure, output a structured result line (Step 7)

## Step 2: Plan → `plan-task`

Call `the groundwork-plan-task workflow` with the task ID from Step 1 in the conversation context.
...
**Interactive mode:** This question is **mandatory** — if you reached here with a plan, you must ask it rather than stopping. Use `Ask the user`:
...
Call `the groundwork-implement-task workflow` with `task_id` and `plan_file_path` in the conversation context.
...
**Call the validate skill directly.** Do NOT wrap this in a sub-task — this skill runs in the main conversation, which CAN spawn the 9 validation sub-tasks it needs.

1. `cd` into the worktree path from Step 3
2. Call: `the groundwork-validate workflow`
