...
> Spawn the `researcher` agent for **researcher** with this task:
>
...
Follow the groundwork-design-it-twice workflow steps: frame the problem, generate 2–3 divergent designs (parallel `Agent` fan-out when available, sequential otherwise — no agent teams needed), compare them on depth / locality / seam, and carry an opinionated recommendation into the decision records. Skip for designs with no non-trivial new interface.

## Step 5: Document Architecture

When all major decisions are made, create the architecture document using template in `<skill-directory>/references/architecture/architecture-template.md`.
...
> Spawn the `prd-architecture-checker` agent for **prd-architecture-checker** with this task:
>
...
- Follow the groundwork-split-architecture workflow steps — this runs automatically with no user input needed.
...
**Record a decision only when it passes the decision-record gate** (`<skill-directory>/references/engineering-principles.md`): hard to reverse, surprising without context, and a real trade-off — otherwise just implement it. Each qualifying decision is captured inline in the architecture doc (no separate ADR files — `housekeeper` keeps these current) using this format:
...
- `<skill-directory>/references/architecture/architecture-template.md` - Template for architecture document
- `<skill-directory>/references/architecture/decision-examples.md` - Example decision records from real projects
