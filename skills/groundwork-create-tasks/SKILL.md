...
description: Generate implementation tasks from architecture document.
---
...
**Important:** When asking questions to the user (milestone validation, task review, scope clarification), always use the `Ask the user` tool with appropriate options.
...
>
> Research Topic: [technology from architecture]
> Research Questions:
...
**Important:** `/groundwork-work-on-next-task` parses these status values exactly. Any variation will cause parse errors.
...
### Vertical Slicing
Prefer tasks that deliver working functionality over horizontal layers (the **thin vertical slice** principle — `<skill-directory>/references/engineering-principles.md`):
...
**Task status table in `_index.md`** (required — used by `/groundwork-work-on-next-task` to find work without reading every task file):
```markdown
### Task Status
...
You MUST call the skill `the groundwork-task-validation-loop workflow` now.
...
- `<skill-directory>/references/tasks/tasks-template.md` - Template for tasks document
