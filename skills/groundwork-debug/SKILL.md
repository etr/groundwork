...
Use that **literal resolved path** everywhere for the rest of the session — including in every sub-task prompt — so the path can never drift between derivations, and **return it in your final output and any handoff** so the next session can adopt it directly.
