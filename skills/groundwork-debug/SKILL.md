...
When dispatching sub-tasks for parallel investigation:
1. **Pass context forward** — Include files already read and findings so far in the sub-task prompt, not just "investigate X"
...
4. **Reference the debug journal** — Pass the **literal resolved journal path** (not the `{slug}` pattern) and tell the sub-task to read it first so it doesn't repeat prior work
5. **Validate findings** — If a sub-task returns without direct evidence (only file reads, no command output), run the reproduction yourself before accepting its conclusions
...
2. **Write a failing test** — Follow the groundwork-test-driven-development workflow steps and follow the TDD cycle. The test must reproduce the exact bug.
...
Maintain a persistent debug file at `{{debug_dir}}/{slug}.md`. This survives context management and provides continuity across sessions.
...
**Derive the slug once, at the start of Phase 1:** kebab-case of the bug's one-line description, truncated to 40 characters (e.g. "login timeout on token refresh" → `login-timeout-on-token-refresh`). Compute the resolved journal path from it, then use that **literal path** everywhere for the rest of the session — including in sub-task prompts — so the path can never drift between derivations.
...
| "The sub-task explored the codebase thoroughly" | Did it run the failing operation? Reading files is Phase 1, not Phase 3. |
...
- Spawning investigation sub-tasks without specifying what to execute
