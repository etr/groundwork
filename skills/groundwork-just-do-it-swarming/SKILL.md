...
description: Execute all remaining tasks using agent teams for context isolation. Each task runs in its own session.
---
...
Executes all remaining tasks using **agent teams** for context isolation. Each task runs in a separate teammate session with its own context window. Teammates are full Claude Code sessions that can spawn sub-tasks (Plan, task-executor, 9 validators), solving the context accumulation problem of inline batch execution.
...
**Model:** Teammates spawn with GLM for extended context.
...
- Each teammate can spawn sub-tasks (Plan, task-executor, 9 validation agents)
...
- Branch and working directory: resolved per task via the worktree-identity helper (`node <skill-directory>/scripts/worktree-identity.js TASK-NNN`) — project-qualified in monorepos (`task/<project>/TASK-NNN`, `<repo>/.worktrees/<project>-TASK-NNN`)
...
1. **Find the skill file:** Use Glob to find `**/work-on/SKILL.md` within the groundwork plugin directory.
2. **Read the file:** Use the Read tool to read the entire file.
3. **Extract the body:** Skip the YAML frontmatter (the `---` delimited block at the top of the file) and keep everything after it. This is the work-on workflow content.
...
4. **Spawn a teammate** with GLM to execute the task. The teammate's spawn prompt must include:

   ```
   You are executing a single task as part of an automated batch run.

   GROUNDWORK_BATCH_MODE=true — do NOT use Ask the user. Proceed automatically.
...
7. **Verify workflow compliance:** Parse the teammate's completion message for validation evidence.
   ...
   - If the validation line is **missing**: send the teammate a correction via `SendMessage`:
     > "Your completion report is missing the VALIDATION line with per-agent iteration counts. Run the validation loop (Step 7.5 of the work-on workflow) and report back with:
     > VALIDATION: code-quality=[N] test-quality=[N] security=[N] spec-alignment=[N] architecture=[N] simplification=[N] housekeeping=[N] performance=[N] design-consistency=[N]"
...
2. **Process one level at a time:**
   ...
   c. **Spawn teammates** for each task in the level, each with GLM and the same spawn prompt as sequential mode.
