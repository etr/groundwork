...
- Treat `GROUNDWORK_BATCH_MODE=true` as authoritative: do not call `Ask the user`.
...
4. **Keep context lean.** Do not read file contents you won't use directly. Pass file paths to sub-tasks and let them read in their own context windows.
...
4. **Verify spec paths exist** (use Glob, not Read):
   ...
   **If specs missing:** Report which are missing and suggest running `/groundwork-design-product` or `/groundwork-design-architecture` first. In runner mode, return `RESULT: FAILURE` instead of asking. Otherwise use `Ask the user` to confirm proceeding without them.
...
2. **Clarify requirements:** Call `the groundwork-understanding-feature-requests workflow`
...
> **Plan TASK-NNN:**
>
> Plan the implementation of a task.
>
> TASK DEFINITION:
...
>
> CONSTRAINTS:
> 1. Work happens in an isolated git worktree — the plan must instruct the implementer to resolve it with `node <skill-directory>/scripts/worktree-identity.js <task-id>` and use the returned `path` and `branch` verbatim (never hand-derived `.worktrees/TASK-NNN` paths)
...
>
> CONSTRAINTS:
> 1. Work happens in an isolated git worktree — the plan must instruct the implementer to resolve it with `node <skill-directory>/scripts/worktree-identity.js <feature-id>` and use the returned `path` and `branch` verbatim (never hand-derived `.worktrees/FEATURE-slug` paths)
> 2. Implementation must follow TDD — write failing tests first, then make them pass
> 3. Plan covers implementation only — validation and merge are handled separately by the caller
> 4. If a design system is present, the plan must reference design tokens, colors, and component patterns from it
>
