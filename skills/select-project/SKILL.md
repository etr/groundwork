...
Selection scope: in a terminal pane, the selection is pane-scoped, survives `/clear`, and other panes are unaffected. Where the harness has no pane identity (chat-window UIs), every selector invocation prints a `groundwork-project-selection-v1` receipt; harnesses that deliver the tool response to the PostToolUse pin hook pin that receipt to the chat's own session snapshot, so concurrent chats keep independent selections. On targets without that hook payload support (manual runs, hookless exports), only a workspace-level default shared across chats exists — treat the selection as conversation-scoped: state the active project in your reply, and pass it explicitly on later invocations (`--project <name>` where plan-task, implement-task, and validate support it, or `GROUNDWORK_PROJECT=<name>` for lib scripts).
