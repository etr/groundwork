...
- `/groundwork-design-product` - For deliberate, interactive requirement creation
- `/groundwork-source-architecture-from-code` - PRD changes may trigger architecture updates

When both `/groundwork-source-product-specs-from-code` and `/groundwork-source-architecture-from-code` run:
1. Run `/groundwork-source-product-specs-from-code` first (product drives architecture)
2. Note any new requirements that may need architectural support
3. Run `/groundwork-source-architecture-from-code` with awareness of PRD changes
