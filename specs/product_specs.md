...
A recovery session SHALL run inside the selected task worktree under the per-task project lease, with no repository gate held across it. The runner SHALL NOT create preservation or rescue snapshots, police repository boundaries, or restore rejected mutations around a recovery session. Repair changes SHALL remain in the selected task worktree for the retried phase and its normal commit flow, and repair sessions SHALL NOT publish.

Accepted repair changes SHALL be regular files inside the selected task worktree, left uncommitted for the retried phase and its normal commit flow. A recovered task SHALL resume the normal lifecycle, publish only after successful validation and finalization, and SHALL NOT prevent later selected tasks from continuing sequentially in the same run. Exhausted recovery or a hard boundary SHALL preserve the task branch and worktree and return an actionable diagnostic.
